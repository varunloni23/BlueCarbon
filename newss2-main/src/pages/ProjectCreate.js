  ...
  const handleSubmit = async () => {
    try {
      ...
      const projectData = {
        ...
        user_id: getUserInfo()?.user_id || 'UNKNOWN_USER',
        user_email: getUserInfo()?.email || 'unknown@example.com'
      };
              ...
              {formData.ecosystem_type && (
                <Card sx={{ mt: 2 }}>
                  <CardContent>
                    ...
                    <img 
                      src={`http://localhost:8002/images/${formData.ecosystem_type === 'salt_marsh' ? 'saltmarsh' : formData.ecosystem_type === 'coastal_wetland' ? 'coastalwetland' : formData.ecosystem_type === 'coral_reef' ? 'coralreef' : formData.ecosystem_type}.png`}
                      alt={`${formData.ecosystem_type} ecosystem`}
                      style={{ 
                        width: '100%', 
                        maxHeight: '200px', 
                        objectFit: 'cover', 
                    ...
                    <Typography variant="caption" color="textSecondary" sx={{ mt: 1, display: 'block' }}>
                      This image will be automatically associated with your project for buyers to see.
                    </Typography>
                  </CardContent>
                </Card>
              )}
